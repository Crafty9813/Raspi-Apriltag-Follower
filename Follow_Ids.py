...
fx = 600.0 #standard values needa tune later on
fy = 600.0
cx = 320.0
cy = 240.0
camera_params = (fx, fy, cx, cy)

TAG_DELAY = 1.5  # In sec
...
while True:
    ...
    if state == "FIND_1":
        ...
        elif time.time() - last_seen_tag_1 > TAG_DELAY:
            state = "FIND_2"
        ...
        if len(detections) > 0 and detections[0].tag_id == 2:
            ...
            tag_center = (int(detections[0].center[0]), int(detections[0].center[1]))
            follow_april_tag(tag_center, frame_center)
        elif time.time() - last_seen_tag_2 > TAG_DELAY:
            state = "FIND_3"
        ...
        if len(detections) > 0 and detections[0].tag_id == 3:
            ...
            tag_center = (int(detections[0].center[0]), int(detections[0].center[1]))
            follow_april_tag(tag_center, frame_center)
        elif time.time() - last_seen_tag_3 > TAG_DELAY:
            stop_motors()
            print("finished")
